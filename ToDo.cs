...
async Task NavigateToTodoMVC(IPage page)
{
	await page.GotoAsync("https://demo.playwright.dev/todomvc/#/");
	await Assertions.Expect(page.GetByRole(AriaRole.Heading, new() { Name = "todos" })).ToBeVisibleAsync();
}

async Task VerifyPageElements(IPage page)
{
	await page.GetByText("Double-click to edit a todo").ClickAsync();
	await page.GetByText("Created by Remo H. Jansen").ClickAsync();
}

async Task InteractWithTodos(IPage page)
{
...
async Task VerifySnapshots(IPage page)
{
